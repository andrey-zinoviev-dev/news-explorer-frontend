...
function Header (props) {
    ...
    return (
        ...
        {props.loggedIn ? <><a href="/" className={props.mainPage && clickedMenuLogo? "header__link header__link_type_small-header" : "header__link header__link_type_saved-news"}>Главная</a><a href="/saved-news" className={props.mainPage? "header__link" : "header__link header__link_type_saved-news header__link_status_inactive"}>Сохраненные статьи</a></> : <a className="header__link header__link_status_active header__link_status_inactive">Главная</a>}
        {props.loggedIn ? <button className={props.mainPage ? "header__login header__login_type_logged-in" : "header__login header__login_type_saved-news header__login_type_logged-in"}>Грета <img src={props.mainPage? logoutImage : logoutImageBlack}></img></button> : <button className="header__login" onClick={props.onLoginButtonClick}>Авторизоваться</button>}
        </nav> : ""}
        </div>
        </div>
