        ...
        <section className={props.mainPage ? "cards" : "cards cards_type_saved-news"}>
            <div className="container">
                {props.isPreloaderActive ? <Preloader /> : <div className={!props.articlesNotFound ? "cards__wrapper" : "cards__wrapper cards__wrapper_search_empty"}>
                        ...
                        {!props.newsApiServerError ? !props.articlesNotFound ? props.mainPage ? (<h2 className="cards__heading">Результаты поиска</h2>) : ("") : <h3 className="cards__heading_search_empty">Ничего не найдено</h3> : ""}
                        {!props.newsApiServerError ? !props.articlesNotFound ? "" : <p className="cards__span">К сожалению по вашему запросу ничего не найдено.</p> : <p className="cards__span cards__span_type_server-error">{props.onSubmitError}</p>}
                        {!props.newsApiServerError ? <ul className="cards__list">
                        ...
                        {!props.newsApiServerError ? !props.articlesNotFound && !moreButtonDisabled? <button className={props.mainPage ? "cards__resubmit" : "cards__resubmit cards__resubmit_type_saved-news"} onClick={showMorePosts}>Показать еще</button> : "" : ""}
                </div>}
            </div>
        </section>
    )
